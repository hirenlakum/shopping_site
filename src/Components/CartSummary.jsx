...
const CartSummary = () => {
 ...
 const getData = JSON.parse(data)
  ...
  {FinalItems.map((price)=>{

    mrp+=price.finalPrice
...
let navigation2 = useNavigate()

const toorderplace = () => {
 navigation1("/orderplace")
}
    ...
    return(
       ...
       <div className="bg-gray-100 rounded-md p-4 h-max">
                    ...
                    <ul className="text-gray-800 mt-6 space-y-3">
                        <li className="flex flex-wrap gap-4 text-sm">Subtotal <span className="ml-auto font-bold">${mrp}</span></li>
                      
                        <hr className="border-gray-300" />
                        <li className="flex flex-wrap gap-4 text-sm font-bold">Total <span className="ml-auto">${totalmrp}</span></li>
                    </ul>

                    <div className="mt-6 space-y-3">
                        <button type="button" className="text-sm px-4 py-2.5 w-full font-semibold tracking-wide bg-gray-800 hover:bg-gray-900 text-white rounded-md" onClick={toorderplace}>Checkout</button>
                        <button type="button" className="text-sm px-4 py-2.5 w-full font-semibold tracking-wide bg-transparent text-gray-800 border border-gray-300 rounded-md" onClick={tocart}>Continue Shopping  </button>
                    </div>
                </div>
       </>
    )
